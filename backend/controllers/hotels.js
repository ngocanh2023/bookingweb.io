...
function removeAccents(str) {
  ...
  const regex = /[àáảãạăằắẳẵặâầấẩẫậèéẻẽẹêềếểễệìíỉĩịòóỏõọôồốổỗộơờớởỡợùúủũụưừứửữựỳýỷỹỵẩẫậằẳẵặẮẰẲẴẶÀÁẢÃẠĂẲ]/g;

  return str.normalize('NFD').replace(regex, (match) => accents[match.charAt(0)]);
}
// exports.postCity = async (req, res, next) => {
//   console.log('body', req.body)
//   const { city, checkIn, checkOut } = req.body;
//   const searchLocation = removeAccents(location.toLowerCase().normalize('NFD'));
//   const searchCheckIn = new Date(checkIn);
//   const searchCheckOut = new Date(checkOut);
//   const searchGuests = parseInt(guests);
//   const filteredHotels = hotels.filter((hotel) => {
//     const hotelLocation = removeAccents(hotel.location.toLowerCase().normalize('NFD'));
//     const hotelPrice = hotel.price;

//     return (
//       hotelLocation.includes(searchLocation) &&
//       hotelPrice >= searchGuests &&
//       hotelPrice <= searchGuests * 3 &&
//       searchCheckIn <= hotel.checkIn &&
//       searchCheckOut >= hotel)
//     })
